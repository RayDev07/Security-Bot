...
module.exports = {
  ...
  execute: async (client, message, args, prefix) => {
    // Code
       ...
       const yes = new client.button().secondary(`uh_yes`, `Yes`, `${client.emoji.tick}`)

    const no = new client.button().secondary(`uh_no`, `No`, `${client.emoji.cross}`)
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === `uh_yes`) {
      interaction.update({
          embeds: [
            new client.emb().desc(
              `${client.emoji.tick} Unhiding all the channels for \`@everyone\``).setAuthor({name: message.author.username, iconURL: message.author.displayAvatarURL({dynamic: true})}).setTimestamp()], components: []})

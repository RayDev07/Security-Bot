...
client.on("channelCreate", async channel => {
  ...
  let logs = await channel.guild.fetchAuditLogs({type: AuditLogEvent.ChannelCreate}).then(x => x.entries.first())
  if (!logs) return;
  let set = await client.db.get(`setup_${channel.guild.id}`)
   ...
   let del = await logs.target.delete().catch(() => { });
              ...
              if (punish == `kick`) {
                ...
                let ch = await channel.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await channel.guild.members.kick(logs.executor.id, {reason: `CodeX Antinuke | Channel Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `CodeX Antinuke | Channel Created Not Whitelisted`}).catch(() => {})

...
client.on("roleCreate", async role => {
  ...
  let logs = await role.guild.fetchAuditLogs({type: AuditLogEvent.RoleCreate}).then(x => x.entries.first())
  let set = await client.db.get(`setup_${role.guild.id}`)
            ...
            let eo = await client.db.get(`exown_${role.guild.id}`) ? await client.db.get(`exown_${role.guild.id}`) : []
   ...
   let del = await logs.target.delete().catch(() => { });
              ...
              if (punish == `kick`) {
                ...
                let ch = await role.guild.channels.fetch(logch).catch(() => {})
                if (!ch || ch === null) return;
                let x = await role.guild.members.kick(logs.executor.id, {reason: `CodeX Antinuke | Role Created Not Whitelisted`}).catch(() => {})
              ...
              if (punish == `mute`) {
                ...
                let x = await mem.timeout(dur, {reason: `CodeX Antinuke | Role Created Not Whitelisted`}).catch(() => {})
